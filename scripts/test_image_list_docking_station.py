...
corners = np.array([[-395.94,-740.60,-607.77], 
                    [-395.94,-740.60, 607.77], 
                    [-395.94, 740.60,-607.77], 
                    [-607.77, 740.60, 607.77], 
                    [395.94, -740.60, -607.77], 
                    [395.94, -740.60, 607.77], 
                    [395.94, 740.60, -607.77], 
                    [395.94,740.60,607.77]])
...
with tf.Session(config=config) as sess:
    ...
    for line in tqdm(lines):
        ...
        #Further Normalization, DEBUG BLOCK 
        ...
        if transform is not None:
            ...
            if args.use_gt:
                # label_file = filename.replace('images', 'labels').replace('.png', '.txt').replace(
				# '.jpg', '.txt').replace('.jpeg', '.txt').strip()
                # if os.path.isfile(label_file):
                # target = line_arr[5:args.nV*2+5]
                target = line_arr[-16:]
                target = [float(x) for x in target]
                box_gt = np.array(target).reshape(args.nV, 2)
                box_gt = np.transpose(box_gt)
                box_gt = np.array(target).reshape(-1, 2)
                ...
                save_img =f'/workspace/data/deepurl_docking_station/test_result/{img_name}_deepurl_result.png' 
                ...
                errs_trans.append(trans_dist/10) #unit adjustment (mm to cm)
                # Compute angle error
                ...
                testing_error_trans += trans_dist/10 #unit adjustment (mm to cm)
...
if args.use_gt:
    ...
    logging.error('   Acc using 10% threshold - {} vx 3D Transformation (ADD) = {:.4f}%'.format(diam * 0.1, acc3d10))
    logging.error('   Acc using 5 cm 5 degree metric = {:.4f}%'.format(acc5cm5deg))
    logging.error('   Acc using 10 cm 10 degree metric = {:.4f}%'.format(acc10cm10deg))
    logging.error('   Acc using 15 cm 15 degree metric = {:.4f}%'.format(acc15cm15deg))
